...
export const createdMockGroupMembers = (users: User[], groups: Group[]): GroupMember[] => {
  const groupMembers: GroupMember[] = []

  groups.filter((g) => !g.is_dm).forEach((group) => {
...
export const createMockMessages = (users: User[], groups: Group[]): Message[] => {
  ...
  groups.forEach((group) => {
    const memberIds = group.is_dm ? ([group.created_by, users.find((u) => u.id !== group.created_by)?.id]
    .filter(Boolean) as string[]) : users.map((u) => u.id)
